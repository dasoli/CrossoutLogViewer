...
namespace CrossoutLogView.Common
{
    ...
    {
        /// <summary>
        ///     Sort a one-dimesional array by swapping each element at <paramref name="items" /> to the index indicated by
        ///     <paramref name="keys" />.
        ...
        public static T[] SortByKeys<T>(this T[] array, int[] keys)
        {
            ...
            for (var i = 0; i < keys.Length; i++) newArray.SetValue(array.GetValue(keys[i]), i);
            return newArray;
        }

        /// <summary>
        ///     Searches for the specified object and returns the zero-based index of the first occurence within the entire <
        ///     <see cref="Array" />.
        /// </summary>
        ...
        {
            for (var i = 0; i < array.Length; i++)
                if (element.Equals(array.GetValue(i)))
                    return i;
            return -1;
        }
    }
