...
namespace CrossoutLogView.Common
{
    public static class CollectionExtention
    {
        ...
        public static void AddRange<TTarget, TSource>(this ICollection<TTarget> target, IEnumerable<TSource> source,
        ...
        public static TCollection AddDays<TCollection>(this TCollection collection, DateTime start, DateTime end)
            where TCollection : ICollection<DateTime>
        {
        ...
        private static void IntrospectiveSort<T>(Collection<T> keys, int left, int length, IComparer<T> comparer)
        {
            ...
            if (length < 0)
                throw new ArgumentOutOfRangeException(nameof(length), "Value must be a non negative integer.");
            if (length > keys.Count)
                throw new ArgumentOutOfRangeException(nameof(length),
                    "Value must be less then the total number of elements in the collection.");
            if (length + left > keys.Count)
                throw new ArgumentException(
                    $"The sum of '{nameof(length)}' and '{nameof(left)}' must be less then the total number of elemments in the collection.");
            if (length < 2) return;
        ...
        private static void IntroSort<T>(Collection<T> keys, int lo, int hi, int depthLimit, IComparer<T> comparer)
        {
            while (hi > lo)
            {
                ...
                var p = PickPivotAndPartition(keys, lo, hi, comparer);
        ...
        {
            var n = hi - lo + 1;
            for (var i = n / 2; i >= 1; i = i - 1) DownHeap(keys, i, n, lo, comparer);
