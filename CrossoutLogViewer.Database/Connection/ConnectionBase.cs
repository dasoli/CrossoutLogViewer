...
namespace CrossoutLogView.Database.Connection
{
    public abstract class ConnectionBase : IDisposable
    {
        ...
        protected void UpdateValues(object obj, VariableInfo[] variableInfos, string tableName, string whereCondition)
        {
            ...
            for (var i = 0; i < variableInfos.Length; i++)
            {
        ...
        protected List<T> ExecuteRequest<T>(string requestCommand,
            TableRepresentation filterTableRepresentation = TableRepresentation.All) where T : new()
        {
            var type = typeof(T);
        ...
        /// <param name="filterTableRepresentation">Masks the <see cref="TableRepresentation" /> of each variable.</param>
        /// <returns>All items in the SQLite database fullfilling the conditions of the REQUEST.</returns>
        protected IList ExecuteRequest(Type type, string requestCommand,
            TableRepresentation filterTableRepresentation = TableRepresentation.All)
        {
            var varInfos = VariableInfo.FromType(type);
            ...
            try
            {
                ...
                while (reader.Read())
                {
                    var obj = ReadCurrentObject(() => Activator.CreateInstance(type), varInfos, reader,
                        filterTableRepresentation);
                    if (obj != null) result.Add(obj);
                }
            }
            catch (InvalidOperationException)
        ...
        protected T ExecuteRequestSingleObject<T>(string requestCommand,
            TableRepresentation filterTableRepresentation = TableRepresentation.All) where T : new()
        {
            var type = typeof(T);
            var varInfos = VariableInfo.FromType(type);
        ...
        /// <param name="filterTableRepresentation">Masks the <see cref="TableRepresentation" /> of each variable.</param>
        /// <returns>The first item in the SQLite database fullfilling the conditions of the REQUEST.</returns>
        protected object ExecuteRequestSingleObject(Type type, string requestCommand,
            TableRepresentation filterTableRepresentation = TableRepresentation.All)
        {
            var varInfos = VariableInfo.FromType(type);
            object result = null;
            try
            {
                ...
                using var reader = cmd.ExecuteReader();
                result = reader.Read()
                    ? ReadCurrentObject(() => Activator.CreateInstance(type), varInfos, reader,
                        filterTableRepresentation)
                    : null;
            }
            catch (InvalidOperationException)
        ...
        /// <returns>
        ///     The double array of rowid references by the field name. Null if the field is not found or the table
        ...
        protected long[] RequestReferences(string condition, Type type, string variableName)
        {
            ...
            try
            {
                ...
                var vi = VariableInfo.FromType(type).FirstOrDefault(x =>
                    x.Name.Equals(variableName, StringComparison.InvariantCultureIgnoreCase));
                if (vi == null) return Array.Empty<long>();
                refs = GetTableRepresentation(vi.VariableType, DatabaseTableTypes) switch
                {
                    TableRepresentation.Reference => new[] { reader.GetInt64(0) },
        ...
        protected object ReadCurrentObject(Func<object> constructor, VariableInfo[] fields, SQLiteDataReader reader,
            TableRepresentation filterTableRepresentations)
        {
            if (constructor is null || fields is null || reader is null) return null;
            var obj = constructor();
            var rowIdIndex = reader.GetOrdinal(RowIdName);
            if (rowIdIndex != -1) RequestRowIds.Add(reader.GetInt64(rowIdIndex));
            foreach (var vi in fields)
            {
                ...
                if (!MaskFilter(varTableRepresentation,
                    filterTableRepresentations)) //fitler doesnt allows variable; skip
                    continue;
                switch (varTableRepresentation)
                {
                    ...
                    case TableRepresentation.Reference:
                        ...
                        vi.SetValue(obj,
                            ExecuteRequestSingleObject(varType, GetRowIdRequestString(varType, reference)));
                        break;
                    case TableRepresentation.StoreArray:
                        var baseType = Types.GetEnumerableBaseType(varType);
                        if (baseType == null)
                            continue;
                        var values = ParseSerializedArray(ReadField<string>(vi.Name, reader),
                            GetDeserializer(baseType));
                    ...
                    case TableRepresentation.ReferenceArray:
                        ...
                        if (baseType == null)
                            continue;
                        var refs = ParseSerializedArray(ReadField<string>(vi.Name, reader), Base85.DecodeInt64);
        ...
        private static readonly ConcurrentDictionary<(Guid GUID, TableRepresentation Included), string>
            requestVariableInfos = new ConcurrentDictionary<(Guid GUID, TableRepresentation Incuded), string>();

        /// <summary>
        ///     Returns the SQLite REQUEST string for a given type, requesting only reference and stored fields, never arrays or
        ///     refernce arrays.
        /// </summary>
        ...
        protected string GetRequestString(Type type, bool includeRowId = false,
            TableRepresentation includedTableRepresentation = TableRepresentation.All)
        {
            ...
            if (!requestVariableInfos.TryGetValue((type.GUID, includedTableRepresentation), out var varInfoString))
            {
                varInfoString = string.Join(", ", includedTableRepresentation == TableRepresentation.All
                    ...
                    : VariableInfo.FromType(type)
                        .Where(x => MaskFilter(GetTableRepresentation(x.VariableType, DatabaseTableTypes),
                            includedTableRepresentation)).Select(x => x.Name.ToLowerInvariant()));
            ...
            return string.Format(FormatRequest, (includeRowId ? RowIdName + ", " : string.Empty) + varInfoString,
                type.Name);
        }
        ...
        protected string GetRowIdRequestString(Type type, long[] rowIds,
            TableRepresentation includedTableRepresentation = TableRepresentation.All)
        {
            if (type is null || rowIds is null) return null;
            var conditions = new string[rowIds.Length];
            var reqTemplate = RowIdName + " == {0}";
            for (var i = 0; i < rowIds.Length; i++) conditions[i] = string.Format(reqTemplate, rowIds[i]);
            return string.Concat(GetRequestString(type, false, includedTableRepresentation), " WHERE ",
        ...
        protected string GetRowIdRequestString(Type type, long rowId,
            TableRepresentation includedTableRepresentation = TableRepresentation.All)
        {
            return GetRequestString(type, false, includedTableRepresentation) +
                   string.Format(" WHERE {0} == {1}", RowIdName, rowId);
        }
        ...
        /// <summary>
        ///     INSERTs a provided object of the type <typeparamref name="T" /> into the database using the
        ...
        /// <param name="filterTableRepresentation">Masks the <see cref="TableRepresentation" /> of each variable.</param>
        protected void Insert<T>(T obj, TableRepresentation filterTableRepresentation = TableRepresentation.All)
            where T : new()
        {
            var type = typeof(T);
        ...
        /// <summary>
        ///     INSERTs a provided object of the <typeparamref name="type" /> into the database using the
        ///     <see cref="SQLiteConnection" />.
        /// </summary>
        /// <param name="obj">The object that will be INSERTed into the database.</param>
        /// <param name="type">Indicates the type of the object, and into which table it will be INSERTed.</param>
        /// <param name="filterTableRepresentation">Masks the <see cref="TableRepresentation" /> of each variable.</param>
        protected void Insert(object obj, Type type,
            TableRepresentation filterTableRepresentation = TableRepresentation.All)
        {
        ...
        /// <summary>
        ///     INSERTs provided objects of the type <typeparamref name="T" /> into the database using the
        ///     <see cref="SQLiteConnection" />.
        /// </summary>
        /// <typeparam name="T">Indicates the type of the object, and into which table it will be INSERTed.</typeparam>
        ...
        protected int InsertMany<T>(IEnumerable<T> objects,
            TableRepresentation filterTableRepresentation = TableRepresentation.All)
        {
        ...
        /// <summary>
        ///     INSERTs provided objects of the type <typeparamref name="type" /> into the database using the
        ///     <see cref="SQLiteConnection" />.
        /// </summary>
        /// <param name="objects">The objects that will be inserted into the database.</param>
        /// <param name="type">Indicates the type of the object, and into which table it will be inserted.</param>
        /// <param name="filterTableRepresentation">Masks the <see cref="TableRepresentation" /> of each variable.</param>
        /// <returns>The number of insertions.</returns>
        protected int InsertMany(IEnumerable objects, Type type,
            TableRepresentation filterTableRepresentation = TableRepresentation.All)
        {
        ...
        /// <summary>
        ///     Handles specific variables where the default behaviour is not appropriate. If the return value is
        ///     <see cref="null" />, the default behaviour will be executed, otherwise the the returned <see cref="Object" /> is
        ///     handled as the value of the variable in context of SQLite database.
        ///     The returned object must be a primitive datatype (integer, floating point, <see cref="String" />), a
        ///     <see cref="DateTime" /> or <see cref="null" />.
        /// </summary>
        /// <param name="obj">
        ...
        ///         <returns>
        ///             <see cref="null" /> if the default behaviour should be executed, otherwise the <see cref="Object" />
        ///             represensting the value of the variable in context of SQLite database.
        ///         </returns>
        protected abstract object InsertVariableHandler(in object obj, in VariableInfo variableInfo);

        private string GenerateInsertValuesString(object obj, VariableInfo[] varInfos,
            TableRepresentation filterTableRepresentation)
        {
            ...
            foreach (var vi in varInfos)
            {
                ...
                switch (varTableRepresentation)
                {
                    ...
                    case TableRepresentation.Reference:
                        ...
                        break;
                    case TableRepresentation.StoreArray:
                        current = SerializeArray(vi.GetValue(obj) as IEnumerable,
                            GetSerializer(Types.GetEnumerableBaseType(vi.VariableType)));
                        break;
                    case TableRepresentation.ReferenceArray:
