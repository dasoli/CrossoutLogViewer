...
namespace CrossoutLogView.Database.Connection
{
    public static class PatchHelper
    {
        public static async IAsyncEnumerable<string> EnumeratePatches(string database, Version minVersion)
        {
            ...
            {
                foreach (var file in Directory.EnumerateFiles(Strings.PatchPath).Where(path =>
                    !path.Contains(Strings.MetadataFile, StringComparison.InvariantCultureIgnoreCase)))
            ...
            {
                await Task.Run(() => Directory.CreateDirectory(Strings.PatchPath));
            }
        }
        ...
        public static bool VerifyPatch(string filePath, string database, Version minVersion)
        {
            ...
            {
                throw new FormatException("Invalid file name format. Invalid version format. filePath = " + filePath,
                    ex);
            }
            ...
            if (string.IsNullOrEmpty(targetDatabase))
                throw new FormatException(
                    "Invalid file name format. Database string cannot be null or whitespace. filePath = " + filePath);
