...
namespace CrossoutLogView.Database.Data
{
    ...
    {
        private static readonly ConcurrentDictionary<int, User> users = new ConcurrentDictionary<int, User>();
        private static readonly SortedSet<int> usersCompleted = new SortedSet<int>();

        private static readonly ConcurrentDictionary<int, long[]> userParticipationRowIds =
        ...
        private static readonly ConcurrentDictionary<DateTime, long> gamesStartRowIds =
            new ConcurrentDictionary<DateTime, long>();

        private static readonly ConcurrentDictionary<long, long[]> gamesPlayerRowIds =
            new ConcurrentDictionary<long, long[]>();

        private static readonly SortedSet<long> gamesCompleted = new SortedSet<long>();

        private static readonly ConcurrentDictionary<string, WeaponGlobal> weapons =
            new ConcurrentDictionary<string, WeaponGlobal>();

        private static readonly SortedSet<string> completedWeapons = new SortedSet<string>();

        private static readonly ConcurrentDictionary<long, GameMap>
            gameMaps = new ConcurrentDictionary<long, GameMap>();

        private static readonly ConcurrentDictionary<string, long> gameMapRowIds =
        ...
        public static IEnumerable<User> GetUsers()
        {
            ...
            statCon.Dispose();
            foreach (var userId in userIds) yield return GetUser(userId);
        }
        ...
        public static User GetUser(int userId)
        {
            ...
            for (var i = 0; i < refs.Length; i++) user.Participations.Add(GetGame(refs[i]));
        ...
        public static bool CompleteUser(User user)
        {
            ...
            using var statCon = new StatisticsConnection();
            Task.WaitAll(
                Task.Run(delegate { user.Weapons = statCon.RequestUserWeapons(user.UserID); }),
                Task.Run(delegate { user.Stripes = statCon.RequestUserStripes(user.UserID); }));
        ...
        {
            foreach (var rowId in rowIds) yield return GetGame(rowId);
        }
        ...
        public static IEnumerable<Game> GetGames(DateTime start, DateTime end)
        {
            ...
            for (var i = 0; i < rowids.Count; i++) yield return GetGame(rowids[i]);
        }
        ...
        {
            if (game == null || !gamesStartRowIds.TryGetValue(game.Start, out var rowId) ||
                gamesCompleted.Contains(rowId)) return false;
            using var statCon = new StatisticsConnection();
            Task.WaitAll(
                Task.Run(delegate
                {
                    ...
                    for (var i = 0; i < playerRowIds.Length; i++)
                        foreach (var vi in VariableInfo
                            .FromType(typeof(Player))
                            .Where(x =>
                                (TableRepresentation.Array &
                                 Serialization.GetTableRepresentation(x.VariableType,
                                     IStatisticData.Implementations)) ==
                                Serialization.GetTableRepresentation(x.VariableType, IStatisticData.Implementations)))
                            vi.SetValue(game.Players[i], vi.GetValue(complement[i]));
                }),
                Task.Run(delegate { game.Rounds = statCon.RequestGameRounds(rowId); }),
                Task.Run(delegate { game.Weapons = statCon.RequestGameWeapons(rowId); }));
        ...
        public static List<WeaponGlobal> GetWeapons()
        {
            ...
            using var statCon = new StatisticsConnection();
            foreach (var name in statCon.RequestWeaponNames()) weapons.Add(GetWeapon(name));
            return weapons;
        }
        ...
        public static bool CompleteWeapon(WeaponGlobal weapon)
        {
            ...
            Task.WaitAll(
                Task.Run(delegate
                {
                    foreach (var uid in statCon.RequestWeaponUserIDs(weapon.Name)) weapon.Users.Add(GetUser(uid));
                }),
                Task.Run(delegate
                {
                    foreach (var rid in statCon.RequestWeaponGamesRowIds(weapon.Name)) weapon.Games.Add(GetGame(rid));
                }));
        ...
        public static GameMap GetMap(string mapName)
        {
            ...
            Task.WaitAll(
                Task.Run(delegate { map = statCon.RequestMap(mapRowId); }),
                Task.Run(delegate
                {
