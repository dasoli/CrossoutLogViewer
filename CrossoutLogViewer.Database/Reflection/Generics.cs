...
namespace CrossoutLogView.Database.Reflection
{
    ...
    {
        private static readonly ConcurrentDictionary<Guid, MethodInfo> generatedGernericIEnumerableCast =
            new ConcurrentDictionary<Guid, MethodInfo>();

        private static readonly ConcurrentDictionary<Guid, MethodInfo> generatedGernericToList =
            new ConcurrentDictionary<Guid, MethodInfo>();

        private static readonly ConcurrentDictionary<Guid, MethodInfo> generatedGernericToArray =
            new ConcurrentDictionary<Guid, MethodInfo>();
        ...
        public static MethodInfo GetEnumerableCast(Type itemType)
        {
            ...
            if (!generatedGernericIEnumerableCast.TryGetValue(itemType.GUID, out var method))
            {
        ...
        {
            //try to get cached or make generic cast method for the type
            if (!generatedGernericToList.TryGetValue(itemType.GUID, out var method))
            {
        ...
        {
            //try to get cached or make generic cast method for the type
            if (!generatedGernericToArray.TryGetValue(itemType.GUID, out var method))
            {
