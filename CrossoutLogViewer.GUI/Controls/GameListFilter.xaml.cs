...
namespace CrossoutLogView.GUI.Controls
{
    ...
    public partial class GameListFilter : ILogging
    {
        ...
        public static (TimeSpan Start, TimeSpan End) AfternoonTimeSpan =
            (new TimeSpan(18, 0, 0), new TimeSpan(21, 59, 59));

        public static readonly DependencyProperty FilterProperty = DependencyProperty.Register(nameof(Filter),
            typeof(GameFilter), typeof(GameListFilter),
            new PropertyMetadata(default(GameFilter), OnFilterPropertyChanged));

        public static readonly DependencyProperty StartLimitProperty = DependencyProperty.Register(nameof(StartLimit),
            typeof(DateTime?), typeof(GameListFilter), new PropertyMetadata(OnStartLimitPropertyChanged));

        public static readonly DependencyProperty EndLimitProperty = DependencyProperty.Register(nameof(EndLimit),
            typeof(DateTime?), typeof(GameListFilter), new PropertyMetadata(OnEndLimitPropertyChanged));

        public static readonly DependencyProperty GameModeProperty = DependencyProperty.Register(nameof(GameMode),
            typeof(GameMode), typeof(GameListFilter), new PropertyMetadata(OnGameModePropertyChanged));
        ...
        /// <summary>
        ///     Gets or sets the default value of <see cref="GameFilter" /> used when initializing a new instance of
        ///     <see cref="GameListFilter" />.
        /// </summary>
        public static GameFilter DefaultFilter { get; protected set; } =
            new GameFilter(GameMode.All, DateTime.Now.StartOfWeek(), DateTime.Now.EndOfWeek());
        ...
        private static void OnFilterPropertyChanged(DependencyObject sender, DependencyPropertyChangedEventArgs e)
        {
            if (sender is GameListFilter cntr && e.NewValue is GameFilter newValue)
            {
                ...
                cntr.FilterChanged?.Invoke(cntr,
                    new ValueChangedEventArgs<GameFilter>(ObjToGameFilter(e.OldValue), newGameFilter));
        ...
        {
            Border_DayFilter.Style =
                Application.Current.Resources[highlighted ? "BorderStyle.Highlight" : "BorderStyle"] as Style;
        }

        private void HighlightSpanGroupBox(bool highlighted)
        {
            Border_SpanFilter.Style =
                Application.Current.Resources[highlighted ? "BorderStyle.Highlight" : "BorderStyle"] as Style;
        }
