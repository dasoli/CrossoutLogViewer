...
namespace CrossoutLogView.GUI.Models
{
    public class PartyGamesModel : StatDisplayViewModeBase
    {
        ...
        public PartyGamesModel(ObservableCollection<UserModel> users, ObservableCollection<GameModel> games)
        {
            Users = users;
            Games = games;
        ...
        {
            get => _unfinished;
            set => Set(ref _unfinished, value);
        }

        public bool UsersExpanded
        {
            get => _usersExpanded;
            set => Set(ref _usersExpanded, value);
        }
        ...
        public static IEnumerable<PartyGamesModel> Parse(IEnumerable<GameModel> games)
        {
            ...
            foreach (var pp in g.Players.GroupBy(x => x.Player.PartyID).Where(x => x.Key != 0))
            {
                ...
                var hash = ArrayHashing.Checksum(userIds);
                // The squad exists
