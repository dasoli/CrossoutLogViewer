...
namespace CrossoutLogView.Updater
{
    public abstract class UpdaterBase
    {
        ...
        protected async Task<FileMetadata[]> GetRemoteMetadata(string filePath)
        {
            ...
            try
            {
                bytes = await Client.Repository.Content.GetRawContent(Strings.RepositoryOwner, Strings.RepositoryName,
                    filePath);
            }
        ...
        protected static FileMetadata[] ComputeMetadataDelta(FileMetadata[] local, FileMetadata[] remote)
        {
            ...
            Console.WriteLine("Updating files:" + Environment.NewLine +
                              string.Join(Environment.NewLine, delta.Select(x => x.Name)));
        ...
        /// <param name="selector">
        ///     The selector used to determine which files to include. If <see cref="null" /> all files are
        ///     included.
        /// </param>
        /// <returns>All files at the specified <paramref name="path" /> in the repository.</returns>
        protected async IAsyncEnumerable<(string Name, T Value)> GetDirectoryContent<T>(string path,
            Func<RepositoryContent, T> converter, Predicate<RepositoryContent> selector = null)
        {
            if (Client is null)
                throw new NullReferenceException("Client cannot be null.");
            var repositoryContents =
                await Client.Repository.Content.GetAllContents(Strings.RepositoryOwner, Strings.RepositoryName, path);
            for (var i = 0; i < repositoryContents.Count; i++)
                if (selector is null || selector(repositoryContents[i]))
                {
                    ...
                    {
                        Console.WriteLine(string.Concat("Message: ", ex.Message, Environment.NewLine, "StackTrace: ",
                            ex.StackTrace, Environment.NewLine, "Source: ", ex.Source));
                    }
        ...
        public async Task GenerateMetadata(string path)
        {
            ...
            var local = FileMetadata.FromPaths(HashFunction, Directory.GetFiles(path))
                .Where(x => !Strings.MetadataFile.Equals(x.Name, StringComparison.InvariantCulture)).ToArray();
            await local.WriteJson(Path.Combine(path, Strings.MetadataFile));
        }

        /// <summary>
        ///     Computes the base64 encoded hash representation of <see cref="byte[] " /> <paramref name="bytes" /> using
        ///     <see cref="SHA1" />.
        ...
        protected static bool UpdateSelector(FileMetadata[] includedFiles, RepositoryContent content)
        {
            ...
            var index = Array.FindIndex(includedFiles,
                x => x.Name.Equals(content.Name, StringComparison.InvariantCulture));
